...
- **Typography Scale Overhaul**: Applied the Golden Scale dynamically across all major layout components including the Hero eyebrow/headline, section headings, body text, about section labels, tickers, research cards, team names/roles/descriptions in the People section, and the active footer's list links, taglines, headers, and copyright texts.
- **Active Footer Fluid Sizing**: Resolved static typography rendering in the active `.footer-inner-card` by overriding static Tailwind classes and hardcoded pixel sizes. Applied `--text-xs`, `--text-sm`, and `--text-md` dynamically to footer headings, links, paragraphs, and copyright blocks.
- **People Section Fluid Sizing**: Restructured typography in the `#people` section, overriding static Tailwind utilities for names (`h3`), roles (`p.font-medium`), and descriptions. Team names now scale fluidly with `--text-md`, roles with `--text-sm`, and description blocks with `--text-base`.
...
- **Adaptive Product Cards & Focal-Point Framing**: Refactored description cards on desktop/tablet to use explicit dynamic height (`clamp(320px, 45vh, 420px)`) so all scrolling product cards remain exactly uniform, with proportional logos (`clamp(45px, 4.5vw, 65px)`) and adaptive font sizes. Shipped custom focal-point alignments (`object-position` overrides) for specific product background slides (e.g. centering the Nanodion athlete on the left side) so subjects never get covered or blocked by overlay cards. Shipped a GPU hardware-accelerated clipping fix (using `-webkit-mask-image` and `isolation: isolate`) on `.product-bg-track` to completely prevent edge-bleeding and layout leakage under all scrolling conditions.
