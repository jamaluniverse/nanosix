document.addEventListener('DOMContentLoaded', () => {
    ...
    // Desktop Mode Detection and Restriction for Mobile Devices
    const overlay = document.getElementById('desktop-mode-restriction');
    ...
    function checkDesktopMode() {
        // Don't show if user already clicked "Continue Anyway" in this session
        if (sessionStorage.getItem('desktop-mode-dismissed') === 'true') {
            return;
        }
        ...
        // Check for iOS desktop mode spoofing (iPad/iPhone reporting MacIntel)
        const isIOSDesktop = (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1);
        
        // Detect if desktop user agent but on a touch-first mobile/tablet screen size
        const isDesktopUA = !/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
        
        // Physical screen dimensions: if either screen side is small (phone/tablet size), it's a mobile device.
        const isSmallScreenDevice = Math.min(window.screen.width, window.screen.height) < 1025;

        // If it's a touch-first small screen device running with a desktop layout
        const isMobileInDesktopMode = isTouch && (isCoarsePointer || isIOSDesktop) && isDesktopUA && isSmallScreenDevice;

        if (isMobileInDesktopMode) {
            // 1. Attempt to force viewport tag back to mobile
            const viewport = document.querySelector('meta[name="viewport"]');
            if (viewport) {
                viewport.setAttribute('content', 'width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no');
            }

            // 2. If the browser ignores the viewport update and window width remains large, show warning overlay
            // Typically desktop mode layout width defaults to 980px or higher
    ...
    // Tab switcher logic
    if (tabIos && tabAndroid && instructionIos && instructionAndroid) {
        tabIos.addEventListener('click', () => {
            tabIos.classList.add('active');
            tabAndroid.classList.remove('active');
            instructionIos.classList.add('active');
            instructionAndroid.classList.remove('active');
        });

        tabAndroid.addEventListener('click', () => {
            tabAndroid.classList.add('active');
            tabIos.classList.remove('active');
            instructionAndroid.classList.add('active');
            instructionIos.classList.remove('active');
        });
    }
    ...
    if (continueBtn && overlay) {
        continueBtn.addEventListener('click', () => {
            ...
            document.body.style.overflow = '';
            sessionStorage.setItem('desktop-mode-dismissed', 'true');
        });
    }
